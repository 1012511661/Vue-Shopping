    ...
    <div id="log">
        <!-- top  -->
          <van-row type="flex" justify="center" >
            <van-col span="18"><p class="title">登录</p></van-col>
        </van-row>
         <!-- 账号 -->
        <van-row type="flex" justify="center" class="name">
            <van-col span="2"><i class="icon iconfont icon-mine"></i></van-col>
            <van-col span="12" offset="1" ><input type="text" placeholder="请输入账号" class="name_input" v-model="name"></van-col>
        </van-row>
        <!-- 密码 -->
           <van-row type="flex" justify="center" class="pwd">
            <van-col span="2"><i class="icon iconfont icon-lock"></i></van-col>
            <van-col span="12" offset="1" ><input type="text" placeholder="请输入密码" class="pwd_input"  v-model="pwd"></van-col>
        </van-row>
        <!-- 滑块 -->
        <van-row type="flex" justify="center" >
          <van-col span="15">
            ...
            <div  class="slider" ref="slider">
              ...
              <!-- 滑块 -->
              <div class="slider_btn" ref="slider_btn" @touchstart='touchStart' @touchmove='touchMove' @touchend='touchEnd'></div>
              <!-- @touchmove='touchMove' @touchend='touchEnd' -->
              <!-- @mouseup="mouseUp($event)" -->
              <!-- 文字 -->
              <div class="slider_text" ref="slider_text">
                  正在验证。。。
              </div>
            </div>
          </van-col>
        ...
        <van-row type="flex" justify="center" class="btn">
            <van-col span="15"  class="log_btn"> <a href="javascript:;" @click="log">登录</a></van-col>
        </van-row>
        <!-- 操作按钮 -->
         <van-row type="flex" justify="center" class="set_btn">
            <van-col span="8"> <span class="reg" @click="go(1)">注册账号</span></van-col>
  ...
  methods: {
    ...
    touchMove(ev) {
      ...
      if (moveX > 0) {
        ...
        sli.style.backgroundColor = `rgba(${this.rgbA[0]},245,${this.rgbA[1]}, ${this.rgbA[2]})`;
        // sliBtn.style.width = moveX + "px";
        if (moveX >= Math.ceil(sli.offsetWidth - sliBtn.offsetWidth)) {
          console.log(moveX);
          console.log(Math.ceil(sli.offsetWidth - sliBtn.offsetWidth));
          this.isSlider = true;
