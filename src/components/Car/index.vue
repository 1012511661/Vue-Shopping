    ...
    <div id="car">
        ...
        <li class="carList_li" v-for="(item,index) in shopList" :key="index">
          <van-row>
            ...
            <van-col span="8" class="list-right">
              ...
              <p style="color:rgb(199, 108, 28)" class="right_size"> {{item.spec}} -  {{item.size}}</p>
            ...
            <van-col span="6" class="list-setBtn">
              ...
              <span class="del"><van-icon name="delete" @click="del(index)"/></span>
            </van-col>
          </van-row>
        </li>
      </ul>
      ...
      <van-row class="bot-grop">
        ...
        <van-col span="8" class="grop-shop">
          ...
          <van-col span="8" class="grop-total"><router-link :to="{path:'/pay'}">去结算</router-link>
        </van-col>  
      </van-row>
      <!-- 弹框 1 -->
      <van-popup v-model="show" position="bottom" class="msg_del">
        <p>是否删除该商品</p>
          <span class="yes"><van-icon name="passed" @click="delY(index)"/></span>
          <span class="no"><van-icon name="close" @click="delN" /></span>
      </van-popup>
      <!-- 弹框 2  -->
       <!-- <van-popup v-model="show" position="bottom" class="msg_del">
        <p></p>
...
export default {
  ...
  computed: {
    ...
    total() {
      ...
      //this.allTotal = Price;
      this.$store.commit("ALL_PRICE", Price);
      return Price;
    },
    //总数
    count() {
  ...
  methods: {
    red(index) {
      //  this.$store.commit("RED_NUM", index);
      //减
  ...
  created() {
    this.axios
      .get(`https://easy-mock.com/mock/5bd6aef0775106311984f5bc/VVant/CarList`)
      .then(res => {
